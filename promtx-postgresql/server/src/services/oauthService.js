...
export class AppleOAuthAdapter {
    ...
    async exchangeCode(code, codeVerifier) {
        ...
        const data = await res.json();
        if (!res.ok)
            throw new Error(`Apple Token Exchange Failed: ${JSON.stringify(data)}`);
        return {
            accessToken: data.access_token,
            refreshToken: data.refresh_token,
            idToken: data.id_token,
...
export class MicrosoftOAuthAdapter {
    ...
    async getUserInfo(tokens) {
        ...
        if (!res.ok)
            throw new Error(`Microsoft Graph Failed: ${JSON.stringify(graphData)}`);
...
export class OAuthService {
    ...
    async authenticateWithProvider(provider, code, state, codeVerifier, appleUser) {
    ...
    async linkProvider(userId, provider, code, codeVerifier) {
        const adapter = this.adapters.get(provider);
        if (!adapter)
            throw new Error(`Unsupported provider: ${provider}`);
        const tokens = await adapter.exchangeCode(code, codeVerifier);
    ...
    async unlinkProvider(userId, provider) {
        ...
        if (!targetAccount)
            throw new Error(`No linked account found for provider ${provider}`);
