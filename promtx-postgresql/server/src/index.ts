...
import { 
  ...
  handleAuthVerifyMfa, handleAuthSetup2fa, handleAuthReferralCode, handleAuthApiKeys,
  handleAuthVerifyToken, handleAuthLogout, handleAuthAvatar, handleAuthForgotPassword, handleAuthResetPassword
} from './routes/auth';
import { 
  handleLlmGenerate, handleLlmGenerateParallel, handleLlmStream, 
  handleLlmAbort, handleLlmUsageHistory, handleLlmUsageSummary 
} from './routes/llm';
import { 
  ...
  handleBillingSubscriptionChange, handleBillingSubscriptionCancel,
  handleBillingSubscriptionResume, handleBillingInvoices,
  handleBillingCreditsUsage
} from './routes/billing';
import { 
  handleAdminUsers, handleAdminFreezeUser, handleAdminUpdateCredits, 
  handleAdminLogs, handleAdminImpersonate 
} from './routes/admin';
import { handleDnaSave, handleDnaQuery } from './routes/dna';
import { 
  handlePromptHistorySave, handlePromptHistoryQuery, handlePromptTemplateSave, 
  handlePromptTemplateQuery, handlePromptTemplateDelete 
} from './routes/prompts';
import { 
  handleImageGenerate, handleImageVariation, handleImageOutpaint, 
  handleImageGallery, handleImagePublic, handleImageUpdatePublic, 
  handleImageShare, handleImageUpscale 
} from './routes/images';
import { 
  handleWorkspaceCreate, handleWorkspaceList, handleWorkspaceInvite, 
...
const rateLimitMap = new Map<string, { count: number, lastAttempt: number, lockedUntil?: number }>();
