...
export async function handleLlmGenerate(req, headers) {
    try {
        ...
        if (!wallet || Number(wallet.credits) <= 0) {
            ...
            if (plan === 'starter') {
                return new Response(JSON.stringify({ error: 'Krediniz bitti. Aboneliğinizi yükseltin.' }), { status: 402, headers: { 'Content-Type': 'application/json', ...Object.fromEntries(headers) } });
            }
            else {
                return new Response(JSON.stringify({ error: 'Krediniz bitti. Ek kredi satın alın.' }), { status: 402, headers: { 'Content-Type': 'application/json', ...Object.fromEntries(headers) } });
            }
        }
