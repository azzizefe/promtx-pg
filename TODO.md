...
> - SQLite `password_hash TEXT NOT NULL` -> Prisma `String?` (OAuth ile kayit olanlarda password olmaz)
...
> **Mevcut:** `src-tauri/src/ipc/commands/auth.rs` — `google_login()` komutu
> **Flow:** Authorization Code + PKCE

- [ ] Mevcut `google_login()` komutunu Account tablosu ile entegre et
- [ ] Google OAuth flow endpoint'leri (web deploy):
...
- [ ] **Tauri (desktop) icin:** Mevcut `google_login()` IPC komutu korunacak, Account tablosu eklenmeli
...
- [ ] **Tauri (desktop) icin:** `apple_login()` IPC komutu olustur (ASWebAuthenticationSession wrapper)
- [ ] **Web icin:** API route endpoint'ler
...
- [ ] Account tablosuna kaydet:
  ...
  ```
- [ ] Session + JWT + RefreshToken olustur
- [ ] **Tauri (desktop) icin:** `microsoft_login()` IPC komutu olustur
- [ ] **Web icin:** API route endpoint'ler
...
- [ ] Multi-Provider OAuth icin: `oauth2` crate veya Bun API layer'da isle (Google, Apple, Microsoft)
...
- [ ] `apple_login` — YENI: Apple Sign-In IPC komutu, Account tablosu, private relay email destegi
- [ ] `microsoft_login` — YENI: Microsoft OAuth IPC komutu, Account tablosu, Graph API entegrasyonu
- [ ] `link_provider` — YENI: Mevcut hesaba OAuth provider baglama
- [ ] `unlink_provider` — YENI: OAuth provider baglantisini kaldirma (min. 1 yontem kontrolu)
- [ ] `get_linked_providers` — YENI: Kullanicinin bagli provider listesi
...
> **Mevcut:** `LoginModal.tsx` — Higgsfield-style SSO-first tasarim:
>   - Ortalanmis "Promtx'e Hos Geldiniz" welcome ekrani
>   - 3 SSO butonu: Google (calisiyor), Apple (stub — `openUrl` ile appleid.apple.com), Microsoft (stub — `openUrl` ile login.microsoftonline.com)
>   - "veya" ayirici + "E-posta ile Devam Et" butonu -> `showEmailForm` toggle
>   - Email formu acilinca: login/register tab'lari, forgot password, sifre guc cubugu (register'da)
>   - Giris yapmissa: hesap paneli (avatar, email, "Cikis Yap" butonu)
>   - Footer: SSO bilgisi + gizlilik politikasi / kullanim kosullari metni
> **Mevcut Rust IPC:** `register`, `login`, `google_login`, `verify_mfa`, `rotate_token`, `setup_2fa`, `create_api_key_cmd`, `verify_token`, `logout`, `impersonate_user`
> **Mevcut Rust altyapi:** rate_limit_service, failed_attempts + account locking (5 deneme / 30dk), 2FA/TOTP, refresh token rotation + reuse detection, welcome email (Resend)
> **Mevcut Store:** `useAppStore` — `isAuthenticated`, `userToken`, `email`, `role`, `id`, `isImpersonated`, `impersonatorId`
...
> **Mevcut Rust:** rate limit 10 istek/email, failed_attempts 5'te 30dk kilit, 2FA check, session 1 saat
...
- [ ] `login` IPC komutu guncelle:
  ...
  - [ ] `isFrozen` kontrolu: donmus hesaplar giris yapamaz (yeni — mevcut Rust'ta yok!)
  ...
  - [ ] Session olustur: `INSERT INTO sessions` (tokenHash, ipAddress, userAgent, deviceFingerprint)
    - [ ] **FIX:** Mevcut Rust'ta IP `"127.0.0.1"` hardcoded, userAgent `"Tauri-Client"` hardcoded — gercek degerler alinmali
    - [ ] Tauri'de: `tauri::window` uzerinden user agent, `req.remote_addr()` uzerinden IP
    - [ ] Web'de: `X-Forwarded-For` header'dan IP, `User-Agent` header'dan device
...
> **Mevcut Rust:** `google_login` — tokeninfo API ile dogrulama, yoksa `register_user` (rastgele sifre ile), refresh token yok
> **Mevcut Frontend:** `handleGoogle` — deep-link + openUrl, 60sn timeout
...
- [ ] `google_login` IPC komutu guncelle:
  ...
  - [ ] `passwordHash` NULL (Google ile giris yapanlar icin sifre yok)
    - [ ] **FIX:** Mevcut Rust'ta rastgele sifre hash'leniyor — NULL olmali (Google-only kullanicilar sifresiz)
  ...
  - [ ] **FIX:** Mevcut Rust'ta refresh token donmuyor (`refresh_token: None`) — donmeli
...
- [ ] Frontend Google login butonu (mevcut `handleGoogle` — korunacak + iyilestirilecek):
  - [ ] Deep-link callback: `deep-link://oauth-callback` (mevcut Tauri)
  - [ ] Web icin: redirect-based OAuth flow (yeni — `window.location.href`)
  - [ ] **FIX:** Mevcut dummy client ID kaldirilmali: `'825946371234-dummy.apps.googleusercontent.com'` -> env'den zorunlu
...
> **Mevcut:** `handleApple` fonksiyonu var ama sadece `openUrl('https://appleid.apple.com')` — STUB
...
- [ ] Frontend Apple login butonu guncelle:
  - [ ] Tauri (desktop): Apple Auth URL ac -> deep-link callback
  - [ ] Web: Apple JS SDK veya redirect flow
  - [ ] Mevcut `handleApple` stub'ini gercek implementasyonla degistir
- [ ] `.env`'ye ekle:
  - [ ] `APPLE_CLIENT_ID=com.promtx.auth`
  - [ ] `APPLE_TEAM_ID=XXXXXXXXXX`
  - [ ] `APPLE_KEY_ID=XXXXXXXXXX`
...
> **Mevcut:** `handleMicrosoft` fonksiyonu var ama sadece `openUrl('https://login.microsoftonline.com')` — STUB
> **Mevcut footer:** "Kurumsal paketler icin SSO kullanilabilir" metni
...
- [ ] `microsoft_login` IPC komutu (yeni):
...
- [ ] Frontend Microsoft login butonu guncelle:
  - [ ] Mevcut `handleMicrosoft` stub'ini gercek implementasyonla degistir
  - [ ] Tauri: openUrl + deep-link callback
  - [ ] Web: redirect flow
- [ ] `.env`'ye ekle:
  - [ ] `MICROSOFT_CLIENT_ID=`
  - [ ] `MICROSOFT_CLIENT_SECRET=`
...
> **Mevcut Rust:** `MagicLinkRequest` import'u var ama implementasyon yok
> **Hedef:** E-posta adresine tek kullanimlik giris linki gonder

- [ ] `send_magic_link` IPC komutu (yeni):
...
- [ ] `verify_magic_link` IPC/API komutu (yeni):
...
- [ ] `logout` IPC komutu guncelle (mevcut `invalidate_all_sessions` var):
...
- [ ] "Tum cihazlardan cikis" secenegi (Settings sayfasinda):
  ...
  - [ ] `invalidate_all_sessions` mevcut IPC'yi kullan
  - [ ] AuditLog: `action: 'user.logout_all_devices'`

#### 16.1.9 Token Yonetimi ve Otomatik Yenileme

> **Mevcut Rust:** `rotate_token` IPC — refresh token rotation + reuse detection (stub implementasyon)
> **Mevcut Frontend:** `apiInvoke()` icinde retry var ama token refresh yok

- [ ] **Refresh token rotation** (mevcut Rust stub'ini tamamla):
...
- [ ] **Frontend otomatik token yenileme:**
  - [ ] `apiInvoke()` icinde 401 response yakalaninca:
    1. `refreshToken`'i store'dan al
    2. `invoke('rotate_token', { refresh_token })` cagir
...
- [ ] **2FA/MFA dialog (mevcut — iyilestir):**
  ...
  - [ ] 6 haneli TOTP kodu input'u (mevcut `verify_mfa` IPC kullan)
...
> **Mevcut Prisma:** `sessions.deviceFingerprint` alani var ama Rust'ta kullanilmiyor
...
- [ ] `get_active_sessions` IPC/API (yeni):
...
| `tokio` | Korunacak |
| `reqwest` | Korunacak (OAuth — Google/Apple/Microsoft, Stripe API) |

---

> **NOT:** Bu dokuman Promtx'in SQLite'tan PostgreSQL'e tam gecisini,
> Prisma ORM'e gecisi, Bun paket yoneticisine gecisi,
> **Multi-Provider OAuth entegrasyonunu (Google + Apple + Microsoft)**,
